	...
	game.onload = function () {					//ロードが終わった後にこの関数が呼び出されるので、この関数内にゲームのプログラムを書こう
		...
		game.onenterframe = function () {
			...
			if (state == 3) {							//状態３（point６以上から）
				zoyamaImg.x += 215;
				zoyamaImg.y = 350 + Math.sin(zoyamaImg.x / 70) * 150; // ｙ座標を振幅100pxのサイン波で移動(Sinは便利なので慣れとくといいよ！)
			}
			...
			if (state == 4.1) {							//状態４．１ 移動状態
				zoyamaImg.x += 300;	
				zoyamaImg.y +=110;					//ただ移動します
			}
			if (state == 5) {							//状態５（point１２以上から）　 ｙ軸が毎フレーム毎に変化する
				zoyamaImg.x += 500;						//移動します。
			...
			if (zoyamaImg.x >= 400) {						//画面端にぞう山画像が行ってしまったら
				...
				//ゲームオーバー後のテキスト表示
				gameOverText.text = "あなたが倒した市野さんは：" + point + "人❤　  　～エキスパートモード～";				//テキストに文字表示 
		...
		endScene.backgroundColor = "purple";
